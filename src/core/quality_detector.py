...
def is_hls_url(url: str) -> bool:
    ...
    Analisa apenas o PATH (a query string costuma carregar tokens onde
    'master'/'playlist' podem aparecer por acaso) e exige que 'master' ou
    'playlist' sejam um componente inteiro do caminho — não um pedaço de
    outra palavra como 'masterclass' ou 'playlists'. Um arquivo de vídeo
    direto (.mp4, .mkv, ...) nunca é tratado como HLS.
    """
    path = urlparse(url).path.lower()
    if ".m3u8" in path:
        return True
