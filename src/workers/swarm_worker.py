...
def _refusal_status(exc: Exception) -> Optional[int]:
    """
    Se a exceção for uma recusa do servidor (401/403/429), devolve o código HTTP.

    Esses erros não passam com retry — só reduzindo o paralelismo. Devolve None
    para erros transitórios (timeout, conexão caiu, range inválido), que valem
    retry normal.
    """
    response = getattr(exc, "response", None)
    code = getattr(response, "status_code", None)
    return code if code in REFUSAL_STATUS_CODES else None
...
def swarm_segment_worker(
    ...
    while attempt <= RETRY_LIMIT and not stop_event.is_set():
        ...
        try:
            ...
            # Recusa do servidor (anti-abuso, ex.: googlevideo). Não adianta
            # repetir aqui — devolve já com o motivo para o orquestrador reduzir
            # o paralelismo e reenfileirar este segmento sem martelar.
            #
            # NÃO mexemos em segment["status"] aqui: o segmento continua 'active'
            # até o orquestrador processar o future e decidir free/failed. Se o
            # worker o marcasse 'free', o get_work() (que roda noutra thread)
            # poderia reagarrar o MESMO segmento antes do orquestrador contabilizar
            # o resultado, gerando submissões duplicadas — exatamente o que
            # quebrava o download sob enxurrada de recusas.
            refusal = _refusal_status(e)
            if refusal is not None:
                segment["fail_reason"] = "refusal"
                segment["fail_status"] = refusal
                return False

            attempt += 1
            if attempt > RETRY_LIMIT:
                segment["fail_reason"] = "transient"
                if log_fn:
                    log_fn(
